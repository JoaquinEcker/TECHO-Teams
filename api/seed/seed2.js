...
const getEquipos = () => {
  return Equipo.findAll().then((eqs) => (equipos = eqs));
};

const getUsuarios = () => {
  return Usuario.findAll().then((usrs) => (usuarios = usrs));
};
...
const addUserToEquipos = async (equipo, num1, num2, token) => {
  for (let i = num1; i < num2; i++) {
    await axios
      .put(
        `http://localhost:3001/api/equipos/${equipos[equipo].id}/${usuarios[i].idPersona}`,
        {},
        { headers: { authorization: token, idPersona: "791718" } }
      )
      .then(() => {
...
const getUsuarioEnEquipo = () => {
  return UsuarioEnEquipo.findAll().then(
    (usuarios) => (usuariosEnEquipos = usuarios)
  );
};

const getRoles = () => {
  return Role.findAll().then((rolesArr) => (roles = rolesArr));
};
...
const agregarRoles = async (userEq, rol, token) => {
  await axios
    .put(
      `http://localhost:3001/api/equipos/${usuariosEnEquipos[userEq].equipoId}/${usuariosEnEquipos[userEq].usuarioIdPersona}/${roles[rol].id}`,
      {},
      { headers: { authorization: token, idPersona: "791718" } }
    )
    .then(() => {
